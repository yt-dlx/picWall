...
function generateCategories(base: typeof metaBase) {
  ...
  Object.keys(base).forEach((parent) => {
    ...
    categoriesArray.push({ name: typedParent, subcategories: [...subCategories, "Everything Mixed"], database: db });
  });
...
const CategoryModal: FC<CategoryModalProps> = ({ isVisible, onClose, onSelectCategory }) => {
  ...
  const generatePreviewLinks = useCallback(() => {
    ...
    rawCategoriesArray.forEach((cat) => {
      ...
      cat.subcategories
        .filter((sub) => sub !== "Everything Mixed")
        .forEach((sub) => {
  ...
  return (
    <Modal visible transparent animationType="slide">
      <View style={{ flex: 1, justifyContent: "flex-end" }}>
        <Animated.View style={[{ transform: [{ translateY }] }, { height: Dimensions.get("window").height * 0.7 }]}>
                  ...
                  {rawCategoriesArray
                    ...
                    .map((category) => (
                      <TouchableOpacity
                        ...
                        className="relative py-10 border-l-4"
                      ...
                      >
                        <Image source={{ uri: previewLinks[category.name] }} style={{ position: "absolute", width: "100%", height: "300%" }} contentFit="cover" />
                        <LinearGradient colors={["transparent", Colorizer("#0C0C0C", 1.0)]} style={{ position: "absolute", width: "100%", height: "300%" }} />
                        <Text
                ...
                <ScrollView className="h-full">
                  <View className="flex-row flex-wrap">
                    {rawCategoriesArray
                      ...
                      ?.subcategories.sort((a, b) => (a === "Everything Mixed" ? -1 : b === "Everything Mixed" ? 1 : 0))
                      .map((child) => (
                        <TouchableOpacity
                          key={child}
                          onPress={() => {
                            setSelectedSubcategory(child);
                            onSelectCategory(activeParent, child === "Everything Mixed" ? "Everything Mixed" : child);
                            handleClose();
                          }}
                          className="w-2/4 p-1"
                        >
                          <View className={`rounded-2xl overflow-hidden border aspect-square ${selectedSubcategory === child ? "border-2 border-[#FFFFFF]" : "border-white/20"}`}>
                            {child === "Everything Mixed" ? (
    ...
    <Animated.View style={fadeInStyle}>
      ...
      <View style={{ marginTop: 30, paddingRight: 6, paddingLeft: 6, paddingBottom: 10 }}>
        ...
        <View className="flex-row justify-center">
          ...
          <CategoryButton category="Categories and Styles" selected={false} onPress={() => setModalVisible(true)} />
        </View>
...
export default function HomePage(): JSX.Element {
  ...
  const fetchData = useCallback(() => {
    ...
    if (selectedChild === "Everything Mixed") {
