...
import { Entypo, MaterialIcons, Ionicons, FontAwesome5, FontAwesome6 } from "@expo/vector-icons";
...
const CategoryModal: FC<CategoryModalProps> = ({ isVisible, onClose, selectedCategory, onSelectCategory }) => {
  const selectedCategoryData = rawCategoriesArray.find((cat) => cat.name === selectedCategory);
  ...
  const generatePreviewLinks = useCallback(() => {
    ...
    selectedCategoryData.subcategories.forEach((subCat) => {
      const subObj = selectedCategoryData.database[subCat];
      if (subObj) {
        const envEntries = Object.values(subObj);
        if (envEntries.length && envEntries[0].images.length) newLinks[subCat] = createPreviewLink(envEntries[0].images[0]);
      }
    });
  ...
  }, [generatePreviewLinks, isVisible]);
  if (!modalVisible || !selectedCategoryData) return null;
  return (
    <Modal visible transparent animationType="none">
      <MotiView from={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ type: "timing", duration: 300 }} style={StyleSheet.absoluteFill}>
        <BlurView intensity={80} style={StyleSheet.absoluteFill} tint="dark" />
        <MotiView
          from={{ translateY: Dimensions.get("window").height }}
          animate={{ translateY: 0 }}
          transition={{ type: "spring", damping: 20, mass: 0.8, stiffness: 100 }}
          style={{ height: Dimensions.get("window").height * 0.9, backgroundColor: Colorizer("#0C0C0C", 0.95), borderTopLeftRadius: 24, borderTopRightRadius: 24, overflow: "hidden" }}
        >
          <View className="p-6 border-b border-white/20">
            <View className="flex-row justify-between items-center">
              <View className="flex-row items-center space-x-3">
                <MaterialIcons name="style" size={28} color="#FFFFFF" />
                <Text className="text-3xl text-white" style={{ fontFamily: "Lobster_Regular" }}>
                  {selectedCategory} Styles
                </Text>
              </View>
              <TouchableOpacity onPress={onClose} className="rounded-full p-2 bg-white/10">
          ...
          <ScrollView className="flex-1 p-4">
            <View className="flex-row flex-wrap justify-between">
              {selectedCategoryData.subcategories.map((style, index) => (
                <MotiView
                ...
                >
                  <TouchableOpacity
                    onPress={() => {
                      ...
                      onSelectCategory(selectedCategory, style);
                      onClose();
                    }}
                    className="relative overflow-hidden rounded-xl border-2"
                    style={{ borderColor: selectedStyle === style ? "#FFFFFF" : "rgba(255,255,255,0.1)", aspectRatio: 3 / 4 }}
                  >
                    {style === "Mixed-All" ? (
                      <Image source={require("@/assets/images/shuffle.gif")} style={{ width: "100%", height: "100%" }} contentFit="cover" />
                    ) : (
                      <MotiView animate={{ scale: [1, 1.1, 1] }} transition={{ loop: true, type: "timing", duration: 5000, repeatReverse: true }} style={{ width: "100%", height: "100%" }}>
                        <Image source={{ uri: previewLinks[style] }} style={{ width: "100%", height: "100%" }} contentFit="cover" />
                      </MotiView>
                    )}
                    <LinearGradient colors={["transparent", Colorizer("#0C0C0C", 0.9)]} style={StyleSheet.absoluteFill} />
                    <View className="absolute bottom-0 w-full p-4 space-y-2">
                      <View className="flex-row items-center space-x-2">
                        <Entypo name="image" size={18} color="#FFFFFF" />
                        <Text className="text-white text-lg" style={{ fontFamily: "Lobster_Regular" }}>
                          {style}
                        </Text>
                      </View>
                      <View className="flex-row items-center space-x-2">
                        <MaterialIcons name="collections" size={16} color="rgba(255,255,255,0.7)" />
                        <Text className="text-white/70 text-sm" style={{ fontFamily: "Dm_Serif_Display_Regular" }}>
                          {Object.keys(selectedCategoryData.database[style] || {}).length} Sets
                        </Text>
                      </View>
                    </View>
                  </TouchableOpacity>
...
const CategoryButton: FC<CategoryButtonExtendedProps> = memo(({ category, selected, onPress }) => {
  ...
  const updateCategoryImage = useCallback(() => {
    if (category === "Shuffle Wallpapers") return;
    const catObj = rawCategoriesArray.find((c) => c.name === category);
    if (!catObj) return;
  ...
  return (
    <TouchableOpacity
      onPress={onPress}
      style={{ height: 80, minWidth: 150, marginHorizontal: 4, borderWidth: 1, borderColor: selected ? "#FFFFFF" : Colorizer("#FFFFFF", 0.1), borderRadius: 10, overflow: "hidden" }}
    >
      <View style={{ borderRadius: 4, overflow: "hidden", width: "100%", height: "100%" }}>
        ...
        <View style={{ position: "absolute", width: "100%", height: "100%", justifyContent: "center", alignItems: "center", flexDirection: "row", borderRadius: 10 }}>
          <FontAwesome6 name="image" size={16} color="#FFFFFF" style={{ marginRight: 8 }} />
...
/* ============================================================================================ */
/* ============================================================================================ */
const FilterStylesButton: FC<{ onPress: () => void }> = memo(({ onPress }) => (
  <TouchableOpacity
    onPress={onPress}
    style={{ backgroundColor: Colorizer("#FFFFFF", 0.1), padding: 12, borderRadius: 8, marginHorizontal: 8, marginBottom: 8, flexDirection: "row", alignItems: "center", justifyContent: "center" }}
  >
    <FontAwesome5 name="filter" size={16} color="#FFFFFF" style={{ marginRight: 8 }} />
    <Text style={{ fontFamily: "Lobster_Regular", color: "#FFFFFF", fontSize: 16 }}>Filter Styles</Text>
  </TouchableOpacity>
));
FilterStylesButton.displayName = "FilterStylesButton";
/* ============================================================================================ */
/* ============================================================================================ */
const HeaderComponent: FC<{
  ...
  selectedCategory: ParentKey | "Shuffle Wallpapers";
  onSelectCategory: (parent: ParentKey | "Shuffle Wallpapers", child?: string) => void;
}> = memo(({ selectedCategory, onSelectCategory, onSearch, onOpenModal }) => {
      ...
      <View style={{ marginTop: 30, paddingRight: 6, paddingLeft: 6, paddingBottom: 10 }}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginVertical: 8 }}>
          ...
          {rawCategoriesArray
            ...
            .map((category) => (
              <CategoryButton key={category.name} category={category.name} selected={selectedCategory === category.name} onPress={() => onSelectCategory(category.name)} />
            ))}
        </ScrollView>
      </View>
    </Animated.View>
  ...
  const renderItem = useCallback(
    ({ item, index }: { item: EnvironmentEntry; index: number }) => (
      <View style={{ flex: 1, margin: 1 }}>
        {index === 0 && selectedParent !== "Shuffle Wallpapers" && <FilterStylesButton onPress={() => setModalVisible(true)} />} <Card data={item} />
      </View>
    ),
    [selectedParent]
  );
  ...
  return (
    <View style={{ backgroundColor: Colorizer("#0C0C0C", 1.0), flex: 1 }} className="relative">
      ...
      <FlatList
        ...
        ListHeaderComponent={<HeaderComponent selectedCategory={selectedParent} onSelectCategory={onSelectCategory} onSearch={handleSearch} onOpenModal={() => setModalVisible(true)} />}
      />
      <CategoryModal isVisible={modalVisible} onClose={() => setModalVisible(false)} selectedCategory={selectedParent} onSelectCategory={onSelectCategory} />
    </View>
  );
}
// /* ============================================================================================ */
